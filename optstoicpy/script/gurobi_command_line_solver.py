...
def solve_with_gurobi_cl_debug(lp_filename, options='Threads=2 TimeLimit=1200 MIPGapAbs=1e-6'):
    args = 'gurobi_cl ' + options + ' ResultFile=%s.sol %s.lp'%(lp_filename, lp_filename)
    command = args.split()
    ...
    if 'Optimal solution found' in output:
        ...
        status_message = [l for l in output.splitlines() if l.startswith('Best')]
        return 'Time_limit', status_message[0]
    else:
        return 'Not_optimal', None


def solve_with_gurobi_cl(lp_filename, options='Threads=2 TimeLimit=1200 MIPGapAbs=1e-6', verbose=True):
    args = 'gurobi_cl ' + options + ' ResultFile=%s.sol %s.lp'%(lp_filename, lp_filename)
    command = args.split()
    process = Popen(command, stdout=PIPE, stderr=PIPE)
    ...
    if 'Optimal solution found' in stdout:
        ...
        status_message = [l for l in stdout.splitlines() if l.startswith('Best')]
        return 'Time_limit', status_message[0]
    else:
        return 'Not_optimal', None
